function loadGraphics()  -- load graphics
  ...
  graphics.setGraphics = function(self, str)
      if str == "mainmenu" then
        self.objects = {}
        local mainmenu = {}
        mainmenu.fontAbductionMed = love.graphics.newFont("abduction/Abduction.ttf", 14)
        mainmenu.fontAbductionBig = love.graphics.newFont("abduction/Abduction.ttf", 18)
        mainmenu.objects = {}
        mainmenu.draw = function(self, x, y)
          love.graphics.setColor(0, 0, 0)
          love.graphics.rectangle("fill", 0, 0, 450, 550)
          love.graphics.setFont(self.fontAbductionBig)
          love.graphics.setColor(0, 255, 0)
          love.graphics.printf("Rebound Shooter: Resurgence", 25, 100, 400, "center")
          love.graphics.setFont(self.fontAbductionMed)
          for i,v in ipairs(self.objects) do
            v:draw(x, y)
          end
        end
        ...
        obj = self:addButton("Play", 100, 250, 1, 250, 0, 0, 255, 200, action)
        table.insert(mainmenu.objects, obj)
        action = function(self)
          -- graphics:setGraphics("credits")
        end
        obj = self:addButton("Credits", 100, 325, 1, 250, 0, 0, 255, 200, action)
        table.insert(mainmenu.objects, obj)
        action = function(self)
          love.event.quit()
        end
        obj = self:addButton("Exit Game", 100, 400, 1, 250, 0, 0, 255, 200, action)
        table.insert(mainmenu.objects, obj)
      ...
      elseif str == "pause" then
        graphics:setGraphics("game")
        local pause = {}
        pause.fontAbductionSm = love.graphics.newFont("abduction/Abduction.ttf", 12)
        pause.fontAbductionMed = love.graphics.newFont("abduction/Abduction.ttf", 14)
        pause.objects = {}
        pause.draw = function(self, x, y)
          ...
          love.graphics.setFont(self.fontAbductionMed)
          love.graphics.printf("Pause", 125, 175, 200, "center")
          love.graphics.setFont(self.fontAbductionSm)
          for i,v in ipairs(self.objects) do
            v:draw(x, y)
          end
        end
        ...
        local obj = self:addButton("Resume", 125, 250, 1, 200, 0, 0, 255, 200, action)
        table.insert(pause.objects, obj)
        action = function(self)
          graphics:setGraphics("toMain")
        end
        obj = self:addButton("Mainmenu", 125, 300, 1, 200, 0, 0, 255, 200, action)
        table.insert(pause.objects, obj)
        action = function(self)
          graphics:setGraphics("toQuit")
        end
        obj = self:addButton("Exit Game", 125, 350, 1, 200, 0, 0, 255, 200, action)
        table.insert(pause.objects, obj)
        table.insert(self.objects, pause)
      elseif str == "toMain" then
        graphics:setGraphics("game")
        local toMain = {}
        toMain.fontAbductionMed = love.graphics.newFont("abduction/Abduction.ttf", 14)
        toMain.fontAbductionSm = love.graphics.newFont("abduction/Abduction.ttf", 12)
        toMain.objects = {}
        toMain.draw = function(self, x, y)
          ...
          love.graphics.setColor(0, 255, 0)
          love.graphics.setFont(self.fontAbductionMed)
          love.graphics.printf("Progress will be Lost", 125, 200, 200, "center")
          love.graphics.setFont(self.fontAbductionSm)
          for i,v in ipairs(self.objects) do
            v:draw(x, y)
          end
        ...
        local obj = self:addButton("Cancel", 125, 375, 1, 85, 255, 255, 255, 200, action)
        ...
        obj = self:addButton("Ok", 240, 375, 1, 85, 255, 0, 0, 200, action)
        table.insert(toMain.objects, obj)
        table.insert(graphics.objects, toMain)
      elseif str == "toQuit" then
        graphics:setGraphics("game")
        local toQuit = {}
        toQuit.fontAbductionMed = love.graphics.newFont("abduction/Abduction.ttf", 14)
        toQuit.fontAbductionSm = love.graphics.newFont("abduction/Abduction.ttf", 12)
        toQuit.objects = {}
  ...
  graphics.addButton = function(self, str, x, y, size, width, r, g, b, opacity, action)
    ...
    obj.draw = function(self, x, y)
      ...
      love.graphics.printf(self.string, self.x, self.y - 8, self.width / ((0.125 * self.size) + 0.875), "center")
    end
